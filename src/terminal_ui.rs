...
pub fn ui<B: Backend>(f: &mut Frame, app: &mut App) {
    ...
    let unfocused_selected_item_style: Style = Style::default().fg(Color::LightCyan);
    ...
    let (player_panel_title, player_panel_text): (String, String) = if let Some((podcast_title, episode_title)) = &app.current_player_episode {
        let current_pos_str: String = format!("{:02}:{:02}", app.current_playback_progress.as_secs() / 60, app.current_playback_progress.as_secs() % 60);
        let total_duration_str: String = app.total_playback_duration.map_or("??:??".to_string(), |d| format!("{:02}:{:02}", d.as_secs() / 60, d.as_secs() % 60));
        let title: String = "Now Playing".to_string();
        let text: String = format!("{} - {} | {} / {} ({})", podcast_title, episode_title, current_pos_str, total_duration_str, player_status_indicator);
        (title, text)
    } else {
        ("Player".to_string(), format!("{} (No episode selected)", player_status_indicator))
    };
    
    let player_widget: Paragraph = Paragraph::new(player_panel_text)
        .wrap(Wrap { trim: true })
    ...
    let podcasts_list_widget: List = List::new(podcasts_list_items)
    ...
    let (episodes_panel_title, episodes_list_items): (String, Vec<ListItem>) = match app.selected_podcast() {
        Some(podcast) => {
            let title: String = format!("Episodes for '{}'", podcast.title());
    ...
    let episodes_list_widget: List = List::new(episodes_list_items)
    ...
    // --- Show Notes Panel ---
    let is_show_notes_focused: bool = app.focused_panel == FocusedPanel::ShowNotes;
    let show_notes_title: String = app.selected_episode().map_or("Show Notes".to_string(), |e| format!("Show Notes: {}", e.title()));
    let show_notes_content: String = app.show_notes_state.content.clone();
    
    let show_notes_widget: Paragraph = Paragraph::new(show_notes_content)
        .wrap(Wrap { trim: true })
    ...
    // --- Hint Bar ---
    let hint_text: &str = "[←/→/Tab] Switch Panel | [↑/↓] Navigate | [Enter] Play | [Space] Pause | [Q] Quit";
    let hint_widget: Paragraph = Paragraph::new(hint_text).style(Style::default().fg(Color::DarkGray)).alignment(Alignment::Center);
